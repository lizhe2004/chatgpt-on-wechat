    ...
    def __init__(self):
        try:
            ...
            if not os.path.exists(config_path):  # 如果没有配置文件，创建本地配置文件
                config = {
                    "speech_synthesis_voice_name": "zh-CN-XiaoyouNeural", # "zh-CN-XiaoxiaoNeural",  # 识别不出时的默认语音
                    "auto_detect": True,  # 是否自动检测语言
                    "speech_synthesis_zh": "zh-CN-XiaoyouNeural", #"zh-CN-XiaozhenNeural",
            ...
            self.speech_config.speech_synthesis_voice_name =  conf().get("azure_speech_synthesis_voice_name_zh") #self.config["speech_synthesis_voice_name"]
    ...
    def textToVoice(self, text):
        if self.config.get("auto_detect"):
            ...
            if key in self.config:
                ...
                logger.info("[Azure] textToVoice auto detect language={}, config voice={}".format(lang,  conf().get("azure_speech_synthesis_voice_name_" + lang)))
